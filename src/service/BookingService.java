...
public class BookingService {
	...
	/**
	 * Lấy danh sách booking của một nhân viên trong khoảng thời gian New method
	 ...
	 */
	public Booking createBooking(int customerId, int petId, int staffId, LocalDateTime bookingTime, String note)
			throws Exception {
	...
	public Booking updateBooking(Booking booking) throws Exception {
		...
		// Kiểm tra dữ liệu đầu vào
		if (booking.getCustomer() == null || booking.getPet() == null || booking.getBookingTime() == null
				|| booking.getStatus() == null) {
			throw new Exception("Dữ liệu không hợp lệ");
		}

		// Kiểm tra xem thời gian đã được đặt chưa (nếu thay đổi thời gian)
		if (booking.getStaff() != null && !booking.getBookingTime().equals(existingBooking.getBookingTime())
	...
	/**
	 * Cập nhật trạng thái booking Modified method to accept StatusEnum directly
	 * instead of String
	 */
	/**
	...
	private void validateStatusChange(StatusEnum currentStatus, StatusEnum newStatus) throws Exception {
		...
		case PENDING:
			...
			if (!StatusEnum.CONFIRMED.equals(newStatus) && !StatusEnum.CANCELLED.equals(newStatus)
		...
		case CONFIRMED:
			...
			if (!StatusEnum.COMPLETED.equals(newStatus) && !StatusEnum.CANCELLED.equals(newStatus)
	...
	private boolean isTimeSlotBooked(int staffId, LocalDateTime bookingTime) throws Exception {
		...
		for (Booking booking : bookings) {
			...
			if ((bookingTime.isEqual(startTime) || bookingTime.isAfter(startTime)) && bookingTime.isBefore(endTime)) {
				return true;
			}
		}
	...
	public int getBookingDuration(int bookingId) throws Exception {
		...
		// Trong thực tế, bạn có thể tính tổng thời lượng của tất cả dịch vụ trong
		// booking
		return 60;
	}

	/**
	 * Tìm kiếm booking theo từ khóa New method implementation (was missing)
	 */
