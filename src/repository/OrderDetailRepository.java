...
public class OrderDetailRepository implements IRepository<OrderDetail> {
	...
	public List<Service> getServicesByOrderId(int orderId) {
		...
		String sql = "SELECT s.* FROM service s " + "JOIN order_detail od ON s.service_id = od.service_id "
	...
	public double calculateOrderTotal(int orderId) {
		double total = 0;
		String sql = "SELECT SUM(od.price * od.quantity) as total " + "FROM order_detail od WHERE od.order_id = ?";
	...
	public int getServiceQuantity(int orderId, int serviceId) {
		int quantity = 0;
		String sql = "SELECT quantity FROM order_detail " + "WHERE order_id = ? AND service_id = ?";
