...
public class CustomerRepository implements IRepository<Customer> {
	...
	public List<Customer> selectAll() {
		...
		String sql = "SELECT p.*, c.point, c.created_at " + "FROM customer c "
	...
	public Customer selectById(int customerID) {
		...
		String sql = "SELECT p.*, c.point, c.created_at " + "FROM customer c "
				+ "JOIN person p ON c.customer_id = p.person_id " + "WHERE c.customer_id = ?";
	...
	public List<Customer> selectByCondition(String condition, Object... params) {
		...
		String sql = "SELECT p.*, c.point, c.created_at " + "FROM customer c "
				+ "JOIN person p ON c.customer_id = p.person_id " + "WHERE " + condition;
	...
	public Customer findByPhone(String phone) {
		Customer ketQua = null;
		String sql = "SELECT p.*, c.point, c.created_at " + "FROM customer c "
				+ "JOIN person p ON c.customer_id = p.person_id " + "WHERE p.phone = ?";
	...
	public int getTotalNewCustomers(String timeUnit) {
		...
		if (timeUnit.equals("WEEK")) {
			// Truy vấn cho WEEK
			query = "SELECT COUNT(*) AS total_new_customers " + "FROM customer "
	...
	public XYChart.Series<String, Number> getCustomerData(String timeUnit) {
		...
		if (timeUnit.equals("WEEK")) {
			// Lấy 4 tuần gần nhất
			query = "SELECT" + "    CONCAT(DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL (n - 1) WEEK), '%d/%m'), ' - ', "
			...
			query = "SELECT YEAR(created_at) AS year, MONTH(created_at) AS month, COUNT(*) AS total_customers "
					+ "FROM customer " + "WHERE YEAR(created_at) = YEAR(CURRENT_DATE) "
					+ "GROUP BY YEAR(created_at), MONTH(created_at) " + "ORDER BY YEAR(created_at), MONTH(created_at)";

		} else if (timeUnit.equals("YEAR")) {
			// Lấy 4 năm gần nhất
			query = "SELECT YEAR(created_at) AS year, COUNT(*) AS total_customers " + "FROM customer "
					+ "WHERE YEAR(created_at) >= YEAR(CURRENT_DATE) - 4 " + "GROUP BY YEAR(created_at) "
