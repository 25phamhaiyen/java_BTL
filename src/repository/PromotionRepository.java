...
public class PromotionRepository implements IRepository<Promotion> {
	...
	private Promotion mapResultSetToPromotion(ResultSet rs) throws SQLException {
		return new Promotion(rs.getInt("promotion_id"), rs.getString("code"), rs.getString("description"),
				rs.getInt("discount_percent"), rs.getDate("start_date").toLocalDate(),
				rs.getDate("end_date").toLocalDate(), rs.getBoolean("active"));
	}
