...
public class NewBookingController {
	...
	public void initialize() {
		...
		if (serviceComboBox != null) {
			serviceComboBox.setConverter(new StringConverter<Service>() {
				...
				public Service fromString(String string) {
					return serviceComboBox.getItems().stream().filter(service -> service.getName().equals(string))
		...
		if (addressField != null)
			addressField.setPromptText("Nhập địa chỉ khách hàng (tùy chọn)");
		if (emailField != null)
			emailField.setPromptText("Nhập email khách hàng (tùy chọn)");
	...
	private void loadServices() {
		try {
			List<Service> services = serviceRepository.selectAll().stream().filter(Service::isActive)
	...
	private boolean checkBookingConflict(LocalDateTime bookingTime) {
		try (Connection connection = DatabaseConnection.getConnection();
				PreparedStatement stmt = connection
						.prepareStatement("SELECT COUNT(*) FROM booking WHERE booking_time = ?")) {
	...
	private void saveBooking() {
		try {
			...
			if (notes.length() > 500) {
				showAlert(Alert.AlertType.WARNING, "Cảnh báo", "Ghi chú quá dài", "Ghi chú không được quá 500 ký tự.");
				return;
			}
						...
						if (result > 0) {
							try (ResultSet rs = stmt.getGeneratedKeys()) {
								if (rs.next()) {
									...
									System.out.println(
											"Tạo thú cưng mới với ID: " + selectedPet.getPetId() + ", Tên: " + petName);
								}
							}
						} else {
			...
			try (Connection connection = DatabaseConnection.getConnection()) {
				...
				try {
					...
					String insertBookingSql = "INSERT INTO booking (customer_id, staff_id, pet_id, booking_time, status, note) "
							+ "VALUES (?, ?, ?, ?, ?, ?)";
					try (PreparedStatement stmt = connection.prepareStatement(insertBookingSql,
							Statement.RETURN_GENERATED_KEYS)) {
	...
	private void handleSubmit() {
		try {
			...
			// Validation tên khách hàng
			if (customerName.isEmpty()) {
				showAlert(Alert.AlertType.WARNING, "Thông báo", "Thiếu thông tin", "Vui lòng nhập tên khách hàng.");
				return;
			}
			if (customerName.length() > 100) {
			...
			// Validation tên thú cưng
			if (petName.isEmpty()) {
				showAlert(Alert.AlertType.WARNING, "Thông báo", "Thiếu thông tin", "Vui lòng nhập tên thú cưng.");
				return;
			}
			if (petName.length() > 50) {
			...
			Service selectedService = serviceComboBox.getValue();
			if (selectedService == null) {
				showAlert(Alert.AlertType.WARNING, "Thông báo", "Thiếu thông tin", "Vui lòng chọn dịch vụ.");
				return;
			}
			if (!selectedService.isActive()) {
			...
			// Validation ngày giờ
			if (bookingDate == null) {
				showAlert(Alert.AlertType.WARNING, "Thông báo", "Thiếu thông tin", "Vui lòng chọn ngày hẹn.");
				return;
			}
			String timeStr = bookingTimeField.getText().trim();
			...
			if (bookingTime.isBefore(LocalTime.of(8, 0)) || bookingTime.isAfter(LocalTime.of(22, 0))) {
				showAlert(Alert.AlertType.WARNING, "Thông báo", "Giờ không hợp lệ", "Giờ hẹn phải từ 8:00 đến 22:00.");
				return;
			}
			...
			String notes = notesAreaNewBooking.getText().trim();
			if (notes.length() > 500) {
				showAlert(Alert.AlertType.WARNING, "Thông báo", "Ghi chú quá dài", "Ghi chú không được quá 500 ký tự.");
				return;
			}

			// Kiểm tra khách hàng
			if (selectedCustomer == null) {
				showAlert(Alert.AlertType.WARNING, "Thông báo", "Thiếu thông tin", "Vui lòng chọn khách hàng.");
				return;
			}
			...
			String message = String.format(
					"Thông tin đặt lịch:\n" + "Khách hàng: %s\n" + "Số điện thoại: %s\n" + "Địa chỉ: %s\n"
							+ "Email: %s\n" + "Thú cưng: %s\n" + "Ngày hẹn: %s\n" + "Giờ hẹn: %s\n" + "Dịch vụ: %s\n"
							+ "Ghi chú: %s\n\n" + "Xác nhận đặt lịch?",
					selectedCustomer.getFullName(), phone, address.isEmpty() ? "Không có" : address,
					email.isEmpty() ? "Không có" : email, selectedPet.getName(),
					bookingDate.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")),
					bookingTime.format(DateTimeFormatter.ofPattern("HH:mm")), selectedService.getName(),
			...
			if (result.isPresent() && result.get() == ButtonType.YES) {
				...
				// Tạo booking
				Booking createdBooking = bookingService.createBooking(selectedCustomer.getId(), selectedPet.getPetId(),
						currentStaff != null ? currentStaff.getId() : 0, bookingDateTime, notes);
			...
			showAlert(Alert.AlertType.ERROR, "Lỗi", "Không thể đặt lịch", "Đã xảy ra lỗi: " + e.getMessage());
			e.printStackTrace();
		}
	}
