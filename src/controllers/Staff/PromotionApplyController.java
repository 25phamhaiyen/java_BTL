...
public class PromotionApplyController implements Initializable {
     ...
     * Combine both updateTotalPrice methods into one
     */
    private void updateTotalPrice() {
        ...
        if (currentInvoice != null && currentInvoice.getSubtotal() != null) {
            ...
            if (finalTotal < 0) finalTotal = 0; // Ensure non-negative
                ...
                if (invoice.getPromotionCode() != null && !invoice.getPromotionCode().isEmpty()) {
                    ...
                    if (appliedPromotion != null) {
                        ...
                        if (invoice.getDiscountAmount() != null) {
                            discountAmountLabel.setText(String.format("%,.0f VND", invoice.getDiscountAmount().doubleValue()));
                        }
                    }
                } else {
                    discountAmountLabel.setText("0 VND");
    ...
    private void applyPoints(ActionEvent event) {
        ...
        if (pointsField.getText().isEmpty()) {
            showAlert(Alert.AlertType.WARNING, "Cảnh báo", "Chưa nhập số điểm", 
                    "Vui lòng nhập số điểm muốn sử dụng.");
            return;
        }
        ...
        try {
            ...
            showAlert(Alert.AlertType.ERROR, "Lỗi", "Số điểm không hợp lệ", 
    ...
    private void saveChanges() {
        ...
        try {
            ...
            if (selectedPromotion != null) {
                invoiceRepository.updateInvoiceDiscount(
            ...
            if (pointsToUse > 0) {
                invoiceRepository.updateInvoicePoints(
                    currentInvoice.getInvoiceId(),
                    pointsToUse,
                    finalTotal
                );
            ...
            showAlert(Alert.AlertType.INFORMATION, "Thành công", "Đã lưu thay đổi", 
                    "Đã cập nhật thông tin khuyến mãi và điểm tích lũy cho hóa đơn #" + 
                    currentInvoice.getInvoiceId());
