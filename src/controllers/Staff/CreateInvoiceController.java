...
public class CreateInvoiceController implements Initializable {
    ...
    @FXML private Label customerPointLabel;
    @FXML private TableView<ServiceRow> invoiceItemsTable;
    ...
    @FXML private TextField pointsUsedField; // Sửa từ pointUsedField
    @FXML private Label pointValueLabel;
    @FXML private Label usePointCheckbox; // Note: Should be CheckBox in FXML
    ...
    private void initializeServicesTable() {
        if (invoiceItemsTable.getColumns().size() >= 6) {
            TableColumn<ServiceRow, Integer> indexCol = (TableColumn<ServiceRow, Integer>) invoiceItemsTable.getColumns().get(0);
            TableColumn<ServiceRow, String> serviceNameCol = (TableColumn<ServiceRow, String>) invoiceItemsTable.getColumns().get(1);
            TableColumn<ServiceRow, Integer> quantityCol = (TableColumn<ServiceRow, Integer>) invoiceItemsTable.getColumns().get(2);
            TableColumn<ServiceRow, Double> priceCol = (TableColumn<ServiceRow, Double>) invoiceItemsTable.getColumns().get(3);
            TableColumn<ServiceRow, Double> totalCol = (TableColumn<ServiceRow, Double>) invoiceItemsTable.getColumns().get(4);
            TableColumn<ServiceRow, Void> deleteCol = (TableColumn<ServiceRow, Void>) invoiceItemsTable.getColumns().get(5);

            // Cột STT
            indexCol.setCellValueFactory(cellData -> new SimpleIntegerProperty(invoiceItemsTable.getItems().indexOf(cellData.getValue()) + 1).asObject());
    ...
    private void updateDiscountAmount() {
        try {
            double discountPercent = discountField.getText().isEmpty() ? 
                    0.0 : Double.parseDouble(discountField.getText());
    ...
    private void updatePointsValue() {
        try {
            int pointsUsed = pointsUsedField.getText().isEmpty() ? 
                    0 : Integer.parseInt(pointsUsedField.getText());
    ...
    private void addServiceToInvoice() {
        try {
            ...
            showAlert(AlertType.ERROR, "Lỗi", "Không thể thêm dịch vụ", 
                    "Đã xảy ra lỗi: " + e.getMessage());
        }
    }
    ...
    private void applyPromotionCode() {
        ...
        if (promotionCode.isEmpty()) {
            showAlert(AlertType.WARNING, "Cảnh báo", "Mã khuyến mãi rỗng", 
        ...
        try {
            ...
            showAlert(AlertType.ERROR, "Lỗi", "Không thể áp dụng mã khuyến mãi", 
                    "Đã xảy ra lỗi: " + e.getMessage());
        }
    }
    ...
    private void processPaymentAndPrint() {
        ...
        showAlert(AlertType.INFORMATION, "Thông báo", "In hóa đơn", 
                "Hóa đơn đã được in.");
    }
