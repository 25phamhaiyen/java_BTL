...
public class BookingViewController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		try {
			...
			currentDateLabel
					.setText("Ngày hiện tại: " + LocalDate.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
	...
	private void initializeBookingTable() {
		...
		petColumn.setCellValueFactory(cellData -> {
			...
			System.out.println("Thú cưng của booking " + booking.getBookingId() + ": ID=" + pet.getPetId() + ", Tên='"
					+ pet.getName() + "'");
		...
		serviceColumn.setCellValueFactory(
				cellData -> new SimpleStringProperty(getServiceNameFromBooking(cellData.getValue())));
	...
	private void initializeUpcomingBookingTable() {
		...
		upcomingTimeColumn
				.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getBookingTime() != null
		...
		upcomingServiceColumn.setCellValueFactory(
				cellData -> new SimpleStringProperty(getServiceNameFromBooking(cellData.getValue())));
	...
	private String getServiceNameFromBooking(Booking booking) {
		...
		try {
			...
			String sql = "SELECT s.name, s.service_id FROM service s "
					+ "JOIN booking_detail bd ON s.service_id = bd.service_id " + "WHERE bd.booking_id = ?";
			...
			while (rs.next()) {
				...
				System.out.println("-> Dịch vụ của booking ID " + booking.getBookingId() + ": '" + serviceName
						+ "', ID=" + serviceId);
			...
			try (PreparedStatement checkStmt = conn
					.prepareStatement("SELECT COUNT(*) FROM booking_detail WHERE booking_id = ?")) {
				checkStmt.setInt(1, booking.getBookingId());
				try (ResultSet checkRs = checkStmt.executeQuery()) {
					if (checkRs.next()) {
						int count = checkRs.getInt(1);
						System.out.println("Số lượng bản ghi trong booking_detail cho booking " + booking.getBookingId()
								+ ": " + count);
					}
				}
			}
	...
	private Booking loadBookingDirectly(int bookingId) {
		...
		try (Connection conn = DatabaseConnection.getConnection()) {
			String sql = "SELECT b.booking_id, b.booking_time, b.status, b.note, "
					...
					+ "p.pet_id, p.name AS pet_name, " + "s.staff_id, sp.full_name AS staff_name " + "FROM booking b "
			...
			try (PreparedStatement stmt = conn.prepareStatement(sql)) {
				...
				try (ResultSet rs = stmt.executeQuery()) {
					if (rs.next()) {
						...
						System.out.println("Đã tải booking " + bookingId + ", Thú cưng: '" + pet.getName() + "', ID: "
								+ pet.getPetId());
